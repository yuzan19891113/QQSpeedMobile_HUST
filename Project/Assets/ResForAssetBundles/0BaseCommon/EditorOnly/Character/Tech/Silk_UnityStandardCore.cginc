...
#include "Silk_UnityPBSLighting.cginc"
#include "UnityStandardUtils.cginc"
#include "UnityGBuffer.cginc"
#include "Silk_UnityStandardBRDF.cginc"
...
{
    //float4 ColTex = tex2D(_MainTex, i_tex.xy);
    float4 NormalTex = tex2D(_BumpMap, i_tex.xy);
    float3 tangentNormal = UnpackNormal(NormalTex);
    tangentNormal.xy *= _BumpScale;
    tangentNormal.z = sqrt(1 - saturate(dot(tangentNormal.xy, tangentNormal.xy)));
    float3x3 T2WMatrix = float3x3(tangentToWorld[0].xyz, tangentToWorld[1].xyz, tangentToWorld[2].xyz);
...
inline FragmentCommonData FragmentSetup (inout float4 i_tex, float3 i_eyeVec, half3 i_viewDirForParallax, float4 tangentToWorld[3], float3 i_posWorld)
{
    ...
    tangent = tangent.x * tangentToWorld[0] + tangent.y * tangentToWorld[1] + tangent.z * tangentToWorld[2];
